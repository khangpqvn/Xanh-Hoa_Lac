...
int nodeMTNewSchedule[11]; /* lịch multi đang xử lý thêm mới */
int currentDeviceStatus = 0; /* trạng thái thực tế thiết bị */
/* DS-1307 */
int currentSecond = 0;
...
void setStatus(int changeTo){
  ...
  if (changeTo == 0){
    ...
    if(checkSession()!=1){
      sms.SendSMS(number, "01. Thiet bi da duoc TAT thanh cong.");
    ...
    setDuration(0);
      ...
      delay(100);
      if (checkStatus()==1 && digitalRead(11)==LOW){
        sms.SendSMS(number, "[TB] Xay ra loi trong qua trinh khoi dong thiet bi. Vui long kiem tra lai.");
        setStatus(0);
        return;
      }
      if (checkSession()!=1){
        sms.SendSMS(number, "01. Thiet bi da duoc BAT thanh cong.");
        flagAssetChecking = true;
      }
  }
}
/* ngắt mạch và thông báo khi có sự cố xảy ra khi thiết bị đang hoạt động */
void checkPowerDuring(){
  if(currentDeviceStatus==1 && checkStatus()==1 && digitalRead(11)==LOW){
    sms.SendSMS(number, "[TB] Xay ra loi trong qua trinh hoat dong. Thiet bi da dung lai. Vui long kiem tra.");
    setStatus(0);
  }
    if (digitalRead(11)==LOW)
      currentDeviceStatus = 0;
    else currentDeviceStatus = 1;
}
...
void resetMTSchedule(int index){
  for(int k=index; k<3; k++)
    for(int i=0; i<11; i++){
      nodeMTSchedule[k][i] = nodeMTSchedule[k+1][i];
      nodeMTSchedule[k+1][i] = 0;
    }
}
...
void primaryAssetChecking(){
  if (flagAssetChecking == true){
    ...
    if (primaryAsset < 10000){
      char smsReply[160] = "[CANH BAO] Tai khoan chinh la ";
      char temp[6];
      sprintf(temp, "%d", primaryAsset);
      strcat(smsReply,temp);
      strcat(smsReply, "d. Vui long nap them tien de duy tri hoat dong.");
      sms.SendSMS(number, smsReply);
    }
...
void setupNode(){
  pinMode(11,INPUT);        //Chân 11 là INPUT
...
void loop() {
  if(started){
    ...
    if (second%5 == 0 && currentSecond != second){
      if(call.CallStatus() == 1) call.HangUp();
      loadNodeSchedule();
        ...
        if(sms.GetSMS(pos, number, smstext, 160)){
          switch (getMode()){
            ...
            default: /* AdSMS, spamSMS, ... */
              sms.SendSMS(number, "Tin nhan sai cu phap.");
          }
        }
